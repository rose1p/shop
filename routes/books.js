...
router.get('/update', function(req, res) {
    ...
    const sql='select *, format(price, 0) fmtprice, date_format(regdate, "%Y-%m-%d") fmtdate from books where bid=?';
    db.get().query(sql, [bid], function(err, rows) {
      if(err) console.log('도서수정......', err);
      res.render('index', {title:'도서수정', pageName:'books/read.ejs', book:rows[0]});
    });
});
...
router.post('/update', function(req, res) {
    ...
    db.get().query(sql, [title, price, authors, publisher, contents], function(err) {
...
router.post('/upload', upload.single('file'), function(req, res) {
    if(req.file) {
      ...
      db.get().query(sql, [image, bid], function() {
